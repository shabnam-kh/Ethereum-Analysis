...
address_val_pair = clean_lines_tra.map(lambda l: (l.split(',')[2], ( float(l.split(',')[3]), time.strftime("%y.%m", time.gmtime(float(l.split(',')[6]))), float(l.split(',')[4]) )))
...
scmamming_status = joined_RDD.filter(is_scamming).map(lambda x: ((x[1][0][1], x[1][1][1]), 1)).reduceByKey(lambda a,b: a+b).sortByKey(ascending=True).saveAsTextFile('scmamming_status') # (time, total_val)
phishing_status = joined_RDD.filter(is_phishing).map(lambda x: ((x[1][0][1], x[1][1][1]), 1)).reduceByKey(lambda a,b: a+b).sortByKey(ascending=True).saveAsTextFile('phishing_status')
fICO_status = joined_RDD.filter(is_fICO).map(lambda x: ((x[1][0][1], x[1][1][1]), 1)).reduceByKey(lambda a,b: a+b).sortByKey(ascending=True).saveAsTextFile('fICO_status')

scmamming_gas = joined_RDD.filter(is_scamming).map(lambda x: (x[1][0][1], x[1][0][2])).reduceByKey(lambda a,b: a+b).sortByKey(ascending=True).saveAsTextFile('scamming_gas') # (time, total_val)
phishing_gas = joined_RDD.filter(is_phishing).map(lambda x: (x[1][0][1], x[1][0][2])).reduceByKey(lambda a,b: a+b).sortByKey(ascending=True).saveAsTextFile('phishing_gas')
fICO_gas = joined_RDD.filter(is_fICO).map(lambda x: (x[1][0][1], x[1][0][2])).reduceByKey(lambda a,b: a+b).sortByKey(ascending=True).saveAsTextFile('fICO_gas')
